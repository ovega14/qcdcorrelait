...
def make_model(
    ...
    if reg_method in TORCH_REGRESSORS.keys():
        ...
        if reg_method == 'MLP':
            ...
            model = CNN(1, 1, hidden_channels=[1], kernel_size=15, 
                        batch_norm=False)
        elif reg_method == 'Transformer':
            model = Transformer(input_dim=1, num_heads=1)
        else:
            raise NotImplementedError(f'Unknown Torch method {reg_method}.')
        ...
        if reg_method == 'LinearRegression':
            ...
            raise NotImplementedError(f'Unknown Sklearn method {reg_method}.')
    else:
        raise NotImplementedError(f'{reg_method} not a known PyTorch or \
                                  Sklearn regressor.')
    return model
...
def train_model(
    ...
    Args:
        ...
        hyperparams: Dictionary of hyperparameters for training
